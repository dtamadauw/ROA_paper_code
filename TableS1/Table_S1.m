...
function demo_epg_steady_state_last8
%Note:  conventional has no slice spoiler; ROA has the additional slice spoiler.
common.NTR_total = 1500;
common.NTR_trace = 8;
common.TR_s = 24.4e-3;
common.T1_s = 1.2;
common.T2_s = 0.1;
...
common.dkRO = 1;
common.GRO_Tpm = 1.0;
common.ampPruneCombined = 1e-14;
common.ampPruneTrace = 1e-14;
common.maxTracePaths = 2e6;
...
outC = epg_steady_state_last8(pC);
outR = epg_steady_state_last8(pR);
writetable(splitComplex(outC.paths),'steady_initialized_pathways_conventional.csv');
writetable(splitComplex(outR.paths),'steady_initialized_pathways_roa.csv');
disp(outC.decompositionCheck);
disp(outR.decompositionCheck);
end

function T = splitComplex(T)
T.AmplitudeReal = real(T.Amplitude);
T.AmplitudeImag = imag(T.Amplitude);
T.Amplitude = [];
end
