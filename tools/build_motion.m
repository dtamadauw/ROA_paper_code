...
%%[r, v] = build_motion(mode, vel_sig, freq, t)
%%
%%INPUT:
%%      mode: motion mode: linear, sine or random
%%      vel_sig: velocity [cm/s] for linear motion, significance of the motion for sine and random motion
%%      freq: frequency of the motion in Hz
%%      FOV: field of view of the simulation
%%      t:  elapsed time from the first excitation pulse 
%%
%%
function [r, v] = build_motion(mode, vel_sig, freq, t)
...
elseif strcmp(mode, 'sine')
    ...
    r = vel_sig*sin(2*pi*freq*t);
    v = vel_sig*2*pi*freq*cos(2*pi*freq*t);
