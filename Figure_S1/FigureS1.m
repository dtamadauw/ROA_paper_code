...
motion = [];
motion.pixScale = 1;
motion.fun = @(t) build_motion('linear', 0, 0, t);
motion.G = 0;
motion.delta_t = 2.56e-3;

moment_y = [0 0.25 0.5 1.0];
F0s = [];
F1s = [];

for ii=1:length(moment_y)
    [F0,F1,~]=RF_spoiling_2D_motion(T1,T2,TR,TE,flip,dphi,Nex,Nf,[2*pi, moment_y(ii)*2*pi],motion,1);
    F0s(ii,:) = F0;
    F1s(ii,:) = F1;
end

okabeIto = {'#E69F00', '#56B4E9', '#009E73', 'k'};
lines = {'-', '-', '-', '--'};
legends = {};
figure; hold on;
for ii=1:length(moment_y)
    plot(abs(F0s(ii,:)), lines{ii}, 'LineWidth', 2.0, 'Color', okabeIto{ii});
    legends{ii} = ['Spoiler moment = ' num2str(moment_y(ii)) '\times 2\pi'];
end
legend(legends); legend('boxoff');
ylabel('Signal Intensity [a.u.]');
xlabel('RF pulse index');
...
figure; hold on;
for ii=1:length(moment_y)
    plot(abs(F1s(ii,:)), lines{ii}, 'LineWidth', 2.0, 'Color', okabeIto{ii});
    legends{ii} = ['Spoiler moment = ' num2str(moment_y(ii)) '\times 2\pi'];
end
legend(legends); legend('boxoff');
ylabel('Signal Intensity [a.u.]');
...
figure; hold on;
for ii=1:length(moment_y)
    plot(angle(F0s(ii,:)), lines{ii}, 'LineWidth', 2.0, 'Color', okabeIto{ii});
    legends{ii} = ['Spoiler moment = ' num2str(moment_y(ii)) '\times 2\pi'];
end
legend(legends); legend('boxoff');
